...
"Everyone knows that debugging is twice as hard as writing a program in the first place. So if you're as clever as you can be when you write it, how will you ever debug it?" according to \myCite{Kernighan.1982}. As this states, debugging is a quite exhausting and time consuming task. For that reason, tools exist to aid the programmer to narrow down the source of the bug for most programming languages. Those tools do this with following methods:
...
The graphics pipeline, also known as rendering pipeline, differentiates from framework to framework but the basic structure of the pipeline within each of them is identical.\myCite{Shirley.2009}
